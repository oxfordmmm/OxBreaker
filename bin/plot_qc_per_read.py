...
def main(FASTQ_FILE: str):
    ...
    READS_QUAL = reads_mean_q(FASTQ_FILE)
    for QUAL in list([10, 15, 20, 25, 30, 35, 40]):
        ax = plot_data(ax, READS_QUAL, QUAL)
        ax_inset = plot_inset(ax_inset, READS_QUAL, QUAL)

    if str('filtered') in FASTQ_FILE:
        plt.savefig(FASTQ_FILE.replace('_filtered.fastq.gz', '_QC_per_read.pdf'),
                    format='pdf', bbox_inches='tight')
    else:
        plt.savefig(FASTQ_FILE.replace('.fastq.gz', '_QC_per_read.pdf'),
                    format='pdf', bbox_inches='tight')
    plt.close('all')
    return
