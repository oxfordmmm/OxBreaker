...
def is_snp(record):
    ...
    alt = record[ALT_ID].replace("<NON_REF>", record[REF_ID])
    #return bool(len(record[REF_ID]) == 1 and len(alt) - alt.count(",") == alt.count(",") + 1)
    return bool(len(record[REF_ID]) == 1 and len(record[ALT_ID]) == 1)
            ...
            for line in vcf_chunk:
                ...
                if not line or line.startswith("#"): # skip empty and commented lines
                    ...
                    if is_anomalous(record, num_samples):
                        ...
                        if is_snp(record) and is_pass(record) and is_pileup(record):
